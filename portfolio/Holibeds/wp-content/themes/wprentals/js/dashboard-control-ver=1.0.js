...
jQuery(document).ready(function ($) {
    ...
    $('.post_review').click(function () {
        var listing_id, ajaxurl, acesta, parent;
    ...
    function create_review_action() {
        ...
        jQuery('#post_review').click(function () {
            var listing_id, ajaxurl, acesta, stars, content, parent;
    ...
    ///////////////////////////////////////////////////////////////////////////////////////
    $('.cancel_own_booking').click(function () {
...
function delete_expense_js() {
    ...
    jQuery('.delete_exp').click(function (event) {
        ...
        if ( isNaN(city_fee) ) {
         ...
         console.log("total_amm "+total_amm +" cleaning_fee "+cleaning_fee +" city_fee "+ city_fee  );
...
function invoice_create_js() {
    ...
    jQuery('#add_inv_expenses').click(function () {
        ...
        if (dashboard_vars.where_currency_symbol === 'before') {
            ex_value_show = dashboard_vars.currency_symbol + ' ' + '<span class="inv_data_value">'+ex_value+'</span>';
        } else {
            ex_value_show = '<span class="inv_data_value">'+ex_value +'</span>' + ' ' + dashboard_vars.currency_symbol;
        }
        total_amm       =   parseFloat(jQuery('#total_amm').attr('data-total'));
        cleaning_fee    =   parseFloat(jQuery('#cleaning-fee').attr('data-cleaning-fee'));
        city_fee        =   parseFloat(jQuery('#city-fee').attr('data-city-fee'));
    ...
    jQuery('#add_inv_discount').click(function () {
        var dis_val, dis_val_show, new_row, total_amm, book_down, cleaning_fee, city_fee, total_amm_compute, deposit, balance;
        dis_val = parseFloat(jQuery('#inv_expense_discount').val(), 10);
        if (dashboard_vars.where_currency_symbol === 'before') {
            dis_val_show = dashboard_vars.currency_symbol + ' ' + '<span class="inv_data_value"> -'+ dis_val+ '</span>';
        } else {
            dis_val_show = '<span class="inv_data_value"> -' +dis_val + '</span> ' + dashboard_vars.currency_symbol;
        }
    ...
    jQuery('#invoice_submit').click(function () {
        var parent, nonce, ajaxurl, bookid, price, details, details_item, acesta;
        details = new Array();
        ajaxurl     =   control_vars.admin_url + 'admin-ajax.php';
        bookid      =   jQuery(this).attr('data-bookid');
        ...
        jQuery('.invoice_content').each(function () {
            ...
            details_item[1] = jQuery(this).find('.inv_data_value').text();
